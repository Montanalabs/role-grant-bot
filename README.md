...
Untrusted input is a command; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — a command
- **Closed type:** `type Decision = GrantRole(Role) | Deny` where `Role` is closed
- **Sink / capability:** `grant assignRole` → `using assignRole { privileged { assignRole(d) } }`
- **Consequence axes:** Trust, Capabilities

## Run the demo

```sh
examples/apps/role-grant-bot/demo.sh
```
...
- `role-grant-bot_safe.eide` — the correct design.
- `role-grant-bot_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
